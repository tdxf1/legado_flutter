...
| 4 | 添加远程书 | `add_remote` | 已实现（BATCH-27c-1 单 server / 27c-3 加多选批量下载 / 27c-4 加排序+搜索） | `context.push('/remote-books')` |
| 5 | 添加网络URL | `add_url` | 灰显（依赖 WebBook 抓详情栈） | 不写 |
| 6 | 扫码导入 | `qr_scan` | flutter 自加保留（与本地书 / 远程书同位） | `context.push('/qr-scan')` |
| 7 | 书架管理 | `bookshelf_manage` | 已实现（BATCH-27d，选择模式 + 4 actionbar 批量删除/允许更新/禁用更新/移到分组/清缓存） | `context.push('/bookshelf-manage')` |
...
| 12 | 导入书架 | `import_bookshelf` | 灰显（依赖 search 栈或简化版） | 不写 |
