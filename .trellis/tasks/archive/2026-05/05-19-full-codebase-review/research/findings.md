...
- **F-W2B-001** WebDAV 凭据明文写到 `webdav.json`（注释自承"先存明文"） (Resolved by BATCH-03，缩范围：仅 WebDAV password 走 secure_storage；F-W1A-020 备份密码留 BATCH-03b)
...
- **[F-W2B-001]** WebDAV 凭据明文写入应用 documents 的 `webdav.json` — `flutter_app/lib/features/settings/webdav_config_page.dart:181-187` (Resolved by BATCH-03，缩范围：仅 WebDAV password 走 secure_storage；F-W1A-020 备份密码留 BATCH-03b)
...
- **[F-W1B-025]** parse_chapters_from_page 每章 extract_from_contexts — `core/core-source/src/parser.rs:1294-1297` (Resolved by BATCH-13，部分：Runtime 创建成本消解；RuleContext clone 重构推 BATCH-13b)
...
- **[F-W2A-012]** AnimatedBuilder 合并 listenable，每帧重建 — `flutter_app/lib/features/reader/page/` (Partial Resolved by BATCH-19c：子项 1 评估保留 Listenable.merge 现状（7 个 notify 点全是离散低频用户/系统事件，拆嵌套 AnimatedBuilder 收益不划算）；子项 2 _calcPoints 早退缓存 + 子项 3 LinearGradient shader 缓存 split-out 留独立 follow-up；spec 写明复评触发条件)
...
- **[F-W2B-009]** rss article detail 找文章用全数组遍历 — `flutter_app/lib/features/rss/article_detail_page.dart` (Resolved by BATCH-21 — Future.wait 并行 source/article/isStarred；FRB 桥优化留 BATCH-21b)
...
- **[F-W2B-012]** RSS optimistic mark_read 与 detail 异步写库时序 — `flutter_app/lib/features/rss/article_list_page.dart` (Resolved by BATCH-21 — 软一致语义文档化；rollback 机制留 future work)
