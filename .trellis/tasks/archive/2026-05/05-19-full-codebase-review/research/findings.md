...
- **F-W1A-056** backup_dao 未处理 WAL sidecar 文件（Open，识别于 BATCH-08c）
...
- **F-W2B-022** 各 feature 各自 `getApplicationDocumentsDirectory()` 拼路径
...
- **F-W2A-008** fontSize 与 readerSettings.fontSize 双 source of truth
...
- **[F-W1A-056]** backup_dao 未处理 -wal/-shm sidecar 文件，启用 WAL 后未 checkpoint 时备份会丢未 commit 改动 — `core/core-storage/src/backup_dao.rs` (Open，识别于 BATCH-08c)
...
- **[F-W2A-008]** fontSize 双 source of truth — `flutter_app/lib/core/providers.dart`
...
- **[F-W2B-022]** 各 feature 自行拼 documents 路径 — cross-feature
